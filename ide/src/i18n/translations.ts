export type Lang = 'en' | 'ru' | 'kk'

export const translations = {
  en: {
    // Sidebar
    dashboard: 'Dashboard',
    admin: 'Admin',
    studentGroups: 'Student Groups',
    profile: 'Profile',
    signOut: 'Sign out',

    // Dashboard
    welcomeBack: 'Welcome back, Developer!',
    pickUp: 'Pick up where you left off, or start something new.',
    courses: 'Courses',
    lessonsCompleted: 'Lessons Completed',
    tasksSolved: 'Tasks Solved',
    allCourses: 'All Courses',
    noCourses: 'No courses yet.',
    couldNotLoad: 'Could not load courses.',

    // Auth
    checkEmail: 'Check your email — magic link sent to',
    sendMagicLink: 'Send magic link',
    sending: 'Sending…',
    orContinueWith: 'or continue with',
    continueWithGitHub: 'Continue with GitHub',

    // Course plan
    backToDashboard: '← Dashboard',
    noModules: 'No modules yet.',
    noModulesAdmin: 'No modules yet. Add them in the Admin panel.',
    loading: 'Loading…',
    lessons: 'lessons',
    goToCourse: 'Go to Course',
    overallProgress: 'Overall Progress',
    completed: 'Completed',
    inProgress: 'In Progress',
    moduleComplete: 'Complete',
    moduleResources: 'Module Resources',
    cheatSheet: 'Cheat Sheet',
    cheatSheetDesc: 'A quick guide to syntax and common functions.',
    codeSandbox: 'Code Sandbox',
    codeSandboxDesc: 'Experiment with your code in a safe environment.',
    startTask: 'Start Task',

    // IDE
    backToTasks: '← Back to tasks',
    run: '▶ Run',
    running: 'Running…',
    light: 'Light',
    dark: 'Dark',
    inputOutput: 'Input / Output',
    constraints: 'Constraints',
    noExamples: 'No examples provided.',
    aiMentor: 'AI Mentor',
    askMentor: 'Ask mentor',
    cancel: 'Cancel',
    voiceOn: 'Voice ON',
    voiceOff: 'Voice OFF',
    subtitles: 'Subtitles',
    analyzingCode: 'Analyzing your code…',
    preparingResponse: 'Preparing response…',
    cancelled: 'Cancelled',
    output: 'Output',
    runToSeeOutput: 'Run your code to see output.',

    // Admin
    courseManagement: 'Course Management',
    manageCourses: 'Manage courses, modules, lessons, and tasks.',
    liveRepositories: 'Live Repositories',
    noCoursesYet: 'No courses yet. Create one →',
    newCourse: 'New Course',
    newModule: 'New Module',
    newLesson: 'New Lesson',
    newTask: 'New Task',
    createCourse: 'Create Course',
    createModule: 'Create Module',
    createLesson: 'Create Lesson',
    createTask: 'Create Task',

    // Admin form labels & messages
    titleLabel: 'Title',
    slugLabel: 'Slug',
    descriptionLabel: 'Description',
    iconLabel: 'Icon (emoji or text)',
    colorLabel: 'Color',
    courseLabel: 'Course',
    moduleLabel: 'Module',
    lessonLabel: 'Lesson',
    moduleTitleLabel: 'Module Title',
    lessonTitleLabel: 'Lesson Title',
    taskTitleLabel: 'Task Title',
    theoryMarkdown: 'Theory (Markdown)',
    supportsMarkdown: 'Supports Markdown with code blocks.',
    languageLabel: 'Language',
    functionNameLabel: 'Function Name',
    functionNameHint: 'JS/Python only. Must match the function in starter code.',
    starterCodeLabel: 'Starter Code',
    testCasesJson: 'Test Cases (JSON)',
    testCasesHint: 'For SQL: add setup_sql and validation_sql keys per test.',
    selectCourseOption: 'Select course…',
    selectModuleOption: 'Select module…',
    selectOption: 'Select…',
    existingLabel: 'Existing:',
    courseCreated: 'Course created!',
    moduleCreated: 'Module created!',
    lessonCreated: 'Lesson created!',
    taskCreated: 'Task created!',
    selectCourseFirst: 'Select a course first',
    selectModuleFirst: 'Select a module first',
    selectLessonFirst: 'Select a lesson first',
    invalidJson: 'Invalid JSON in test cases',

    // Groups
    studentGroupsTitle: 'Student Groups',
    comingSoon: 'Coming soon.',

    // 404
    pageNotFound: 'Page not found',
    pageNotFoundDesc: "The page you're looking for doesn't exist.",
    goHome: 'Go home',
  },

  ru: {
    dashboard: 'Главная',
    admin: 'Админ',
    studentGroups: 'Группы студентов',
    profile: 'Профиль',
    signOut: 'Выйти',

    welcomeBack: 'С возвращением!',
    pickUp: 'Продолжите с того места, где остановились.',
    courses: 'Курсы',
    lessonsCompleted: 'Уроков пройдено',
    tasksSolved: 'Задач решено',
    allCourses: 'Все курсы',
    noCourses: 'Курсов пока нет.',
    couldNotLoad: 'Не удалось загрузить курсы.',

    checkEmail: 'Проверьте почту — ссылка отправлена на',
    sendMagicLink: 'Отправить ссылку',
    sending: 'Отправка…',
    orContinueWith: 'или войти через',
    continueWithGitHub: 'Войти через GitHub',

    backToDashboard: '← Главная',
    noModules: 'Модулей пока нет.',
    noModulesAdmin: 'Модулей пока нет. Добавьте их в панели администратора.',
    loading: 'Загрузка…',
    lessons: 'уроков',
    goToCourse: 'К курсу',
    overallProgress: 'Общий прогресс',
    completed: 'Пройдено',
    inProgress: 'В процессе',
    moduleComplete: 'Завершён',
    moduleResources: 'Ресурсы модуля',
    cheatSheet: 'Шпаргалка',
    cheatSheetDesc: 'Краткий справочник по синтаксису и основным функциям.',
    codeSandbox: 'Песочница',
    codeSandboxDesc: 'Экспериментируйте с кодом в безопасной среде.',
    startTask: 'Начать задачу',

    backToTasks: '← К задачам',
    run: '▶ Запустить',
    running: 'Выполняется…',
    light: 'Светлая',
    dark: 'Тёмная',
    inputOutput: 'Вход / Выход',
    constraints: 'Ограничения',
    noExamples: 'Примеры не указаны.',
    aiMentor: 'ИИ-ментор',
    askMentor: 'Спросить ментора',
    cancel: 'Отмена',
    voiceOn: 'Голос ВКЛ',
    voiceOff: 'Голос ВЫКЛ',
    subtitles: 'Субтитры',
    analyzingCode: 'Анализирую код…',
    preparingResponse: 'Подготовка ответа…',
    cancelled: 'Отменено',
    output: 'Вывод',
    runToSeeOutput: 'Запустите код, чтобы увидеть результат.',

    courseManagement: 'Управление курсами',
    manageCourses: 'Создавайте курсы, модули, уроки и задачи.',
    liveRepositories: 'Курсы',
    noCoursesYet: 'Курсов нет. Создайте первый →',
    newCourse: 'Новый курс',
    newModule: 'Новый модуль',
    newLesson: 'Новый урок',
    newTask: 'Новая задача',
    createCourse: 'Создать курс',
    createModule: 'Создать модуль',
    createLesson: 'Создать урок',
    createTask: 'Создать задачу',

    titleLabel: 'Название',
    slugLabel: 'Слаг',
    descriptionLabel: 'Описание',
    iconLabel: 'Иконка (эмодзи или текст)',
    colorLabel: 'Цвет',
    courseLabel: 'Курс',
    moduleLabel: 'Модуль',
    lessonLabel: 'Урок',
    moduleTitleLabel: 'Название модуля',
    lessonTitleLabel: 'Название урока',
    taskTitleLabel: 'Название задачи',
    theoryMarkdown: 'Теория (Markdown)',
    supportsMarkdown: 'Поддерживает Markdown с блоками кода.',
    languageLabel: 'Язык',
    functionNameLabel: 'Имя функции',
    functionNameHint: 'Только JS/Python. Должно совпадать с функцией в стартовом коде.',
    starterCodeLabel: 'Стартовый код',
    testCasesJson: 'Тест-кейсы (JSON)',
    testCasesHint: 'Для SQL: добавьте ключи setup_sql и validation_sql в каждый тест.',
    selectCourseOption: 'Выберите курс…',
    selectModuleOption: 'Выберите модуль…',
    selectOption: 'Выбрать…',
    existingLabel: 'Существующие:',
    courseCreated: 'Курс создан!',
    moduleCreated: 'Модуль создан!',
    lessonCreated: 'Урок создан!',
    taskCreated: 'Задача создана!',
    selectCourseFirst: 'Сначала выберите курс',
    selectModuleFirst: 'Сначала выберите модуль',
    selectLessonFirst: 'Сначала выберите урок',
    invalidJson: 'Неверный JSON в тест-кейсах',

    studentGroupsTitle: 'Группы студентов',
    comingSoon: 'Скоро.',

    pageNotFound: 'Страница не найдена',
    pageNotFoundDesc: 'Такой страницы не существует.',
    goHome: 'На главную',
  },

  kk: {
    dashboard: 'Басты бет',
    admin: 'Әкімші',
    studentGroups: 'Студент топтары',
    profile: 'Профиль',
    signOut: 'Шығу',

    welcomeBack: 'Қайта келдіңіз!',
    pickUp: 'Тоқтаған жерінізден жалғастырыңыз.',
    courses: 'Курстар',
    lessonsCompleted: 'Өтілген сабақтар',
    tasksSolved: 'Шешілген тапсырмалар',
    allCourses: 'Барлық курстар',
    noCourses: 'Курс жоқ.',
    couldNotLoad: 'Курстарды жүктеу мүмкін болмады.',

    checkEmail: 'Поштаңызды тексеріңіз — сілтеме жіберілді',
    sendMagicLink: 'Сілтеме жіберу',
    sending: 'Жіберілуде…',
    orContinueWith: 'немесе кіру',
    continueWithGitHub: 'GitHub арқылы кіру',

    backToDashboard: '← Басты бет',
    noModules: 'Модульдер жоқ.',
    noModulesAdmin: 'Модульдер жоқ. Оларды Әкімші панелінен қосыңыз.',
    loading: 'Жүктелуде…',
    lessons: 'сабақ',
    goToCourse: 'Курсқа өту',
    overallProgress: 'Жалпы прогресс',
    completed: 'Аяқталды',
    inProgress: 'Орындалуда',
    moduleComplete: 'Аяқталған',
    moduleResources: 'Модуль ресурстары',
    cheatSheet: 'Шпаргалка',
    cheatSheetDesc: 'Синтаксис пен негізгі функцияларға жылдам нұсқаулық.',
    codeSandbox: 'Код зерттеуі',
    codeSandboxDesc: 'Кодыңызды қауіпсіз ортада тексеріңіз.',
    startTask: 'Тапсырманы бастау',

    backToTasks: '← Тапсырмаларға',
    run: '▶ Іске қосу',
    running: 'Орындалуда…',
    light: 'Ашық',
    dark: 'Қараңғы',
    inputOutput: 'Кіріс / Шығыс',
    constraints: 'Шектеулер',
    noExamples: 'Мысалдар көрсетілмеген.',
    aiMentor: 'ЖИ-ментор',
    askMentor: 'Ментордан сұрау',
    cancel: 'Болдырмау',
    voiceOn: 'Дауыс ҚОСУ',
    voiceOff: 'Дауыс ӨШІРУ',
    subtitles: 'Субтитрлер',
    analyzingCode: 'Код талдануда…',
    preparingResponse: 'Жауап дайындалуда…',
    cancelled: 'Болдырылмады',
    output: 'Нәтиже',
    runToSeeOutput: 'Нәтижені көру үшін кодты іске қосыңыз.',

    courseManagement: 'Курстарды басқару',
    manageCourses: 'Курс, модуль, сабақ және тапсырмалар жасаңыз.',
    liveRepositories: 'Курстар',
    noCoursesYet: 'Курс жоқ. Жаңасын жасаңыз →',
    newCourse: 'Жаңа курс',
    newModule: 'Жаңа модуль',
    newLesson: 'Жаңа сабақ',
    newTask: 'Жаңа тапсырма',
    createCourse: 'Курс жасау',
    createModule: 'Модуль жасау',
    createLesson: 'Сабақ жасау',
    createTask: 'Тапсырма жасау',

    titleLabel: 'Атауы',
    slugLabel: 'Слаг',
    descriptionLabel: 'Сипаттама',
    iconLabel: 'Белгіше (эмодзи немесе мәтін)',
    colorLabel: 'Түс',
    courseLabel: 'Курс',
    moduleLabel: 'Модуль',
    lessonLabel: 'Сабақ',
    moduleTitleLabel: 'Модуль атауы',
    lessonTitleLabel: 'Сабақ атауы',
    taskTitleLabel: 'Тапсырма атауы',
    theoryMarkdown: 'Теория (Markdown)',
    supportsMarkdown: 'Код блоктарымен Markdown қолдайды.',
    languageLabel: 'Тіл',
    functionNameLabel: 'Функция атауы',
    functionNameHint: 'Тек JS/Python. Бастапқы кодтағы функциямен сәйкес болуы керек.',
    starterCodeLabel: 'Бастапқы код',
    testCasesJson: 'Тест-кейстер (JSON)',
    testCasesHint: 'SQL үшін: әр тестке setup_sql және validation_sql кілттерін қосыңыз.',
    selectCourseOption: 'Курс таңдаңыз…',
    selectModuleOption: 'Модуль таңдаңыз…',
    selectOption: 'Таңдау…',
    existingLabel: 'Бар:',
    courseCreated: 'Курс жасалды!',
    moduleCreated: 'Модуль жасалды!',
    lessonCreated: 'Сабақ жасалды!',
    taskCreated: 'Тапсырма жасалды!',
    selectCourseFirst: 'Алдымен курс таңдаңыз',
    selectModuleFirst: 'Алдымен модуль таңдаңыз',
    selectLessonFirst: 'Алдымен сабақ таңдаңыз',
    invalidJson: 'Тест-кейстерде JSON қате',

    studentGroupsTitle: 'Студент топтары',
    comingSoon: 'Жақында.',

    pageNotFound: 'Бет табылмады',
    pageNotFoundDesc: 'Мұндай бет жоқ.',
    goHome: 'Басты бетке',
  },
} satisfies Record<string, Record<string, string>>

export type TranslationKey = keyof typeof translations.en
