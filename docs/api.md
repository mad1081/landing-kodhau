...
**Base URL (өндіріс):** `https://kodhau-learning.onrender.com`  
...
### Барлық пайдаланушыларды алу (email бойынша сүзу)
...
**Query параметрлері:** `?email=` — пошта бойынша сүзу (міндетті емес)

**Жауап:** `200 OK`
```json
[
  { "id": "uuid", "email": "user@example.com", "role": "student", "created_at": "..." }
]
```
...
**Рөлге байланысты:** студент — өз топтары, мұғалім — басқарған топтары, admin — барлығы

**Жауап:** `200 OK`
```json
[
  { "id": "uuid", "name": "9A сынып", "teacher_id": "uuid" }
]
```
...
```json
[
  { "id": "uuid", "user_id": "uuid", "email": "student@example.com", "joined_at": "..." }
]
```
...
**Жауап:** `201 Created` — жасалған топ объектісі

---

### Топқа студент қосу (email арқылы)
...
**Жауап:** `201 Created`
```json
{ "ok": true, "user_id": "uuid", "email": "student@example.com" }
```

**Қате:** `404` — пайдаланушы табылмады
